...
local ns = api.nvim_create_namespace("snippet-inserter")
...
local function entrypoint(structure)
	...
	local function update_preview(I, force)
		...
		structure = evaluator.evaluate_structure(inputs)
	...
	api.nvim_buf_set_keymap(input_buf, 'i', '<Tab>', '<Cmd>lua SNIPPETS_FLOATY_HANDLER(1)<cr>', { noremap = true; silent = true })
	api.nvim_buf_set_keymap(input_buf, 'i', '<S-Tab>', '<Cmd>lua SNIPPETS_FLOATY_HANDLER(-1)<cr>', { noremap = true; silent = true })
