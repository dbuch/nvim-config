...
function line:create_item(item)
  local postfix = 'a'
  local prefix = 'b'
  return prefix .. item .. postfix
end

function line.generate_line()
  local segments = function(...)
    table.insert(line, ...)
  end

  local generate = function(_, _)
    return segments
  end
...
require('el').setup({generator = line.generate_line()})
